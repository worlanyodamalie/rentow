<template>
  <div>
    <div class="homepage-container">
      <div class="pv4">
        <nuxt-link :to="backUrl" class="btn btn--grey-outline">
          <svg
            width="7"
            height="10"
            viewBox="0 0 7 10"
          ...
          <div>
            ...
            <p class="mv3 lh-copy fw4 dark">
              ...
              <div class="flex flex-wrap justify-between place-offers">
                <div v-for="(facility, key) in listing.utilities" :key="key">
          ...
          <div>
            <div class="details-card pa4 flex flex-column">
              <h1 class="pv2 green fw7 mb1">
                GHS {{ listing.amount }}
                <span class="green f6">{{ period(listing.period) }}</span>
              </h1>
              ...
              <div>
                ...
                <div>
                  <p class="grey--5 fw4 lh-copy f6">
                    ...
                    planet. They make apartment search so easy. Apartments are
                    as they are in the pictures. They also remove an apartment
                    from here once it is rented out. I love their services.
                  </p>
                </div>
                <div class="flex pv3">
                  <a class="cursor mr2">
              ...
              <div v-if="isAdmin" class="flex bt b--light-gray pt4">
                <button
                  class="btn btn--green-outline cursor mr3"
                  :disabled="
                  ...
                  "
                  @click="
                    approvalStatus(listing.user_id, listing.id, 'REJECTED')
                  "
                >
                  <!-- <Loading v-show="isLoading" /> -->
                  <span>
                    {{ listing.status === "REJECTED" ? "REJECTED" : flagText }}
                  </span>
                </button>
                <button
                  class="btn btn--green"
                  ...
                  "
                  @click="
                    approvalStatus(listing.user_id, listing.id, 'APPROVED')
                  "
                >
                  <!-- <Loading v-show="isLoading" /> -->
                  <span>{{
                    listing.status === "APPROVED" ? "APPROVED" : approvalText
                  }}</span>
                </button>
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      flaggedClicked: false,
      approvalText: "Approve property",
      flagText: "Flag",
    };
  },
  computed: {
  ...
  methods: {
    ...
    async approvalStatus(user_id, property_listing_id, status) {
      try {
        this.isLoading = true;
        const statusObj = {
          user_id: user_id,
        ...
        this.$axios.setHeader("Authorization", token);
        this.$axios.setHeader("Content-Type", "application/json");
        await this.$axios.post("admin/property-listing/approval", statusObj);
        this.isLoading = false;
        this.statusText(statusObj.status);
      } catch (error) {
        console.log("error", error);
      }
    },
    ...
    statusClicked(status) {
      switch (status) {
        case "APPROVED":
          this.approvalClicked = !this.approvalClicked;
          this.flaggedClicked = false;
          this.flagText = "Flag";
          break;
        case "REJECTED":
          this.flaggedClicked = !this.flaggedClicked;
          this.approvalClicked = false;
          this.approvalText = "Approve Property";
          break;
        default:
          break;
      }
